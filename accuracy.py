...
def build_scuc_model(data):
    ...
    u_sto = {(i,t): m.addVar(vtype=cp.COPT.BINARY, name=f'u_sto_{i}_{t}') 
             for i in range(data['sto_num']) for t in range(T_num)}  # 放电状态
    v_sto = {(i,t): m.addVar(vtype=cp.COPT.BINARY, name=f'v_sto_{i}_{t}') 
             for i in range(data['sto_num']) for t in range(T_num)}  # 充电状态
    w_sto = {(i,t): m.addVar(vtype=cp.COPT.BINARY, name=f'w_sto_{i}_{t}') 
             for i in range(data['sto_num']) for t in range(T_num)}  # 停机状态
    
    P_sto = {(i,t): m.addVar(lb=0, ub=data['Pgmax'][i], name=f'P_sto_{i}_{t}') 
             for i in range(data['sto_num']) for t in range(T_num)}  # 储能发电
    P_charge = {(i,t): m.addVar(lb=0, name=f'P_charge_{i}_{t}') 
                for i in range(data['sto_num']) for t in range(T_num)}  # 储能充电
    
    e_sto = {(i,t): m.addVar(lb=0, ub=data['Cm'][i], name=f'e_sto_{i}_{t}') 
             for i in range(data['sto_num']) for t in range(T_num)}  # 储能容量
    
    p_sto = {(i,j,t): m.addVar(lb=0, ub=data['sto_bid_data'][i,j], name=f'p_sto_{i}_{j}_{t}')
             for i in range(data['sto_num']) for j in range(4) for t in range(T_num)}  # 储能分段发电
    ...
    # 储能成本分开计算
    for i in range(data['sto_num']):
        for t in range(T_num):
            # 发电成本
            obj += u_sto[i,t] * data['Pgmin'][i] * data['sto_price'][i,1]
            for j in range(4):
                obj += data['sto_price'][i,j+2] * p_sto[i,j,t]
            
            # 充电成本：创建一个辅助变量来线性化 v_sto[i,t] * P_charge[i,t]
            # 因为P_charge已经在约束中定义为 P_charge[i,t] == v_sto[i,t] * data['Pf'][i]
            obj += data['sto_price'][i,0] * P_charge[i,t]
    ...
    for t in range(T_num):
        m.addConstr(cp.quicksum(P[i,t] for i in range(unit_num)) + 
                   cp.quicksum(P_sto[i,t] for i in range(data['sto_num'])) - 
                   cp.quicksum(P_charge[i,t] for i in range(data['sto_num'])) == load[t])
        
    # 最小开机时间约束
    for i in range(unit_num):
        # 完整时段的约束
        for t in range(T_num + 1 - data['To'][i]):
            sum_u = cp.quicksum(u[i,k] for k in range(t, t+data['To'][i]))
            m.addConstr(sum_u >= data['To'][i] * v[i,t])
        
        # 末尾不足时段的特殊处理
        for t in range(T_num - data['To'][i] + 1, T_num):
            sum_u = cp.quicksum(u[i,k] for k in range(t, T_num))
            remaining_periods = T_num - t
            m.addConstr(sum_u >= remaining_periods * v[i,t])
        
        # 初始时刻约束
        if data['U0'][i] == 1 and data['Tc'][i] < data['To'][i]:
            sum_u = cp.quicksum(u[i,t] for t in range(data['To'][i]-data['Tc'][i]))
            m.addConstr(sum_u == data['To'][i] - data['Tc'][i])

    # 最小停机时间约束
    for i in range(unit_num):
        # 完整时段的约束
        for t in range(T_num + 1 - data['Tf'][i]):
            sum_off = cp.quicksum(1-u[i,k] for k in range(t, t+data['Tf'][i]))
            m.addConstr(sum_off >= data['Tf'][i] * w[i,t])
        
        # 末尾不足时段的特殊处理
        for t in range(T_num - data['Tf'][i] + 1, T_num):
            sum_off = cp.quicksum(1-u[i,k] for k in range(t, T_num))
            remaining_periods = T_num - t
            m.addConstr(sum_off >= remaining_periods * w[i,t])
        
        # 初始时刻约束
        if data['U0'][i] == 0 and data['Tc'][i] < data['Tf'][i]:
            sum_off = cp.quicksum(1-u[i,t] for t in range(data['Tf'][i]-data['Tc'][i]))
            m.addConstr(sum_off == data['Tf'][i] - data['Tc'][i])
    ...
    for t in range(T_num):
        ...
        # 添加带松弛变量的备用约束
        m.addConstr(reserve_capacity + reserve_slack[t] >= SR * data['load'][t], 
        ...
        for s in range(len(Plmax)):
            flow = (cp.quicksum(sens[s,i] * P[i,t] for i in range(unit_num)) + 
                   cp.quicksum(sens[s,-1] * (P_sto[i,t] - P_charge[i,t]) 
                             for i in range(data['sto_num'])) - sum_sens[s,t])
    ...
    # 2. 储能容量变化约束
    for i in range(data['sto_num']):
        m.addConstr(e_sto[i,0] == data['Ci'][i])  # 初始容量
        for t in range(T_num-1):
            m.addConstr(e_sto[i,t+1] == e_sto[i,t] - P_sto[i,t] + P_charge[i,t])
        m.addConstr(e_sto[i,T_num-1] - P_sto[i,T_num-1] + P_charge[i,T_num-1] >= data['Cf'][i])

    # 3. 充放电功率约束
    for i in range(data['sto_num']):
        for t in range(T_num):
            m.addConstr(P_charge[i,t] == v_sto[i,t] * data['Pf'][i])
            m.addConstr(P_sto[i,t] >= data['Pgmin'][i] * u_sto[i,t])
            m.addConstr(P_sto[i,t] <= data['Pgmax'][i] * u_sto[i,t])

    # 储能最小持续时间约束
    for i in range(data['sto_num']):
        # 初始状态约束
        # 最小放电时间
        m.addConstr(cp.quicksum(u_sto[i,t] for t in range(min(data['Tg'][i], T_num))) >= 
                    data['Tg'][i] * u_sto[i,0])
        
        # 最小充电时间
        m.addConstr(cp.quicksum(v_sto[i,t] for t in range(min(data['Tp'][i], T_num))) >= 
                    data['Tp'][i] * v_sto[i,0])
        
        # 最小停机时间
        m.addConstr(cp.quicksum(w_sto[i,t] for t in range(min(data['Ts'][i], T_num))) >= 
                    data['Ts'][i] * w_sto[i,0])

        # 中间时段约束
        for t in range(1, T_num):
            # 最小放电时间约束
            if t + data['Tg'][i] - 1 <= T_num:
                # 修改这里，确保range不会超出T_num
                end_t = min(t + data['Tg'][i], T_num)
                m.addConstr(cp.quicksum(u_sto[i,k] for k in range(t, end_t)) >= 
                          data['Tg'][i] * (u_sto[i,t] - u_sto[i,t-1]))
            
            # 最小充电时间约束
            if t + data['Tp'][i] - 1 <= T_num:
                # 修改这里，确保range不会超出T_num
                end_t = min(t + data['Tp'][i], T_num)
                m.addConstr(cp.quicksum(v_sto[i,k] for k in range(t, end_t)) >= 
                          data['Tp'][i] * (v_sto[i,t] - v_sto[i,t-1]))
            
            # 最小停机时间约束
            if t + data['Ts'][i] - 1 <= T_num:
                # 修改这里，确保range不会超出T_num
                end_t = min(t + data['Ts'][i], T_num)
                m.addConstr(cp.quicksum(w_sto[i,k] for k in range(t, end_t)) >= 
                          data['Ts'][i] * (w_sto[i,t] - w_sto[i,t-1]))

        # 末尾时段约束
        # 放电状态
        for t in range(max(1, T_num - data['Tg'][i] + 2), T_num):
            m.addConstr(cp.quicksum(u_sto[i,k] for k in range(t, T_num)) >= 
                       (T_num - t) * (u_sto[i,t] - u_sto[i,t-1]))
        
        # 充电状态
        for t in range(max(1, T_num - data['Tp'][i] + 2), T_num):
            m.addConstr(cp.quicksum(v_sto[i,k] for k in range(t, T_num)) >= 
                       (T_num - t) * (v_sto[i,t] - v_sto[i,t-1]))
        
        # 停机状态
        for t in range(max(1, T_num - data['Ts'][i] + 2), T_num):
    ...
    m.setParam(cp.COPT.Param.DivingHeurLevel, 1)   # 加强深度优先启发式
    ...
    for t in range(T_num):
        m.addConstr(cp.quicksum(P[i,t] for i in range(unit_num)) + 
                   cp.quicksum(P_sto[i,t] for i in range(data['sto_num'])) - 
                   cp.quicksum(P_charge[i,t] for i in range(data['sto_num'])) + 
...
def solve_scuc():
    ...
    try:
        ...
        # 优化求解参数设置
        model.setParam(cp.COPT.Param.TimeLimit, 3500)  # 时间限制55分钟
        model.setParam(cp.COPT.Param.RelGap, 1e-6)    # 相对间隙改为1e-6
        
        # MIP启动设置
        model.setParam(cp.COPT.Param.MipStartMode, 1)      # 启用MIP启动
        model.setParam(cp.COPT.Param.MipStartNodeLimit, 10) # 限制MIP启动节点数
...
def save_solution(model, data, filename):
    ...
    with open(filename, 'w') as f:
        # 写入目标值，使用科学计数法保持精度
        f.write(f"# Objective value = {model.objval:.8e}\n")
        
        # 储能机组结果
        for i in range(data['sto_num']):
            for t in range(data['T_num']):
                # 储能状态
                if model.getVarByName(f'u_sto_{i}_{t}').x > 0.5:  # 发电
                    f.write(f"storage{i+1}_s_{t} 1\n")
                    f.write(f"storage{i+1}_p_{t} {model.getVarByName(f'P_sto_{i}_{t}').x:.8e}\n")
                elif model.getVarByName(f'v_sto_{i}_{t}').x > 0.5:  # 充电
                    f.write(f"storage{i+1}_s_{t} -1\n")
                    f.write(f"storage{i+1}_p_{t} {-model.getVarByName(f'P_charge_{i}_{t}').x:.8e}\n")
                else:  # 停机
                    f.write(f"storage{i+1}_s_{t} 0\n")
                    f.write(f"storage{i+1}_p_{t} 0\n")
        
        # 常规机组结果
        for i in range(data['unit_num']):
            for t in range(data['T_num']):
                status = model.getVarByName(f'u_{i}_{t}').x
                power = model.getVarByName(f'P_{i}_{t}').x
                f.write(f"unit{i+1}_s_{t} {int(round(status))}\n")
                f.write(f"unit{i+1}_p_{t} {power:.8e}\n")
